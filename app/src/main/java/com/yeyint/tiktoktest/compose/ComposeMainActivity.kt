package com.yeyint.tiktoktest.compose

import android.content.Context
import android.os.Bundle
import android.util.AttributeSet
import android.view.View
import androidx.appcompat.app.AppCompatActivity
import androidx.compose.foundation.layout.Arrangement
import androidx.compose.foundation.layout.Column
import androidx.compose.foundation.layout.fillMaxSize
import androidx.compose.foundation.layout.padding
import androidx.compose.material.Button
import androidx.compose.material.MaterialTheme
import androidx.compose.material.Surface
import androidx.compose.material.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Alignment
import androidx.compose.ui.Modifier
import androidx.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import com.yeyint.tiktoktest.R
import com.yeyint.tiktoktest.compose.ui.PlantDetailDescription
import com.yeyint.tiktoktest.databinding.ActivityComposeMainBinding

class ComposeMainActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val binding = ActivityComposeMainBinding.inflate(layoutInflater).apply {
            composeMain.setContent {
                MaterialTheme{
                    App(modifier = Modifier.fillMaxSize())
                }
            }
        }
        setContentView(binding.root)
    }

    @Composable
    fun App(modifier: Modifier = Modifier){
        Surface(modifier,color = MaterialTheme.colors.background) {
            Column(modifier = modifier.fillMaxSize(),
                verticalArrangement = Arrangement.Center,
                horizontalAlignment = Alignment.CenterHorizontally) {
                PlantDetailDescription()
                Button(
                    modifier = Modifier.padding(vertical = 24.dp),
                    onClick = {}
                ) {
                    Text("Continue")
                }
            }
        }
    }

    @Preview
    @Composable
    fun MyAppPreview(){
        App(modifier = Modifier.fillMaxSize())
    }
}